...
import tmp from 'tmp';
import { copyCodeAndCreatePullRequest, copyExists, newCmd } from "./copy-code";
...
export async function copyNewCode(sourceRepo: string, octokitParams: OctokitParams, configsStore: ConfigsStore, logger=console): Promise<void> {
    // Clone the source repo.
    const workDir = tmp.dirSync().name;
    const cmd = newCmd(logger);
    const sourceDir = path.join(workDir, 'source');
    cmd(
      `git clone --depth 100 "https://github.com/${sourceRepo}.git" ${sourceDir}`
    );
    ...
    // Iterate over the commit hashes from newest to oldest.
    for (const commitHash of commitHashes) {
        const commitText = cmd(`git log -1 --pretty=oneline ${commitHash}`, {cwd: sourceDir}).toString("utf8");
        ...
        const repos = await configsStore.findReposAffectedByFileChanges(touchedFiles);
