...
export function newFakeOctokit(pulls?: FakePulls, issues?: FakeIssues, default_branch = 'main'): OctokitType {
    return {
...
export function newFakeOctokitFactory(octokit?: OctokitType, token = "b3"): OctokitFactory {
    const factory: OctokitFactory = {
        getGitHubShortLivedAccessToken: () => Promise.resolve(token),
        getShortLivedOctokit: (token?: string) => Promise.resolve(octokit ?? newFakeOctokit())
    };
    return factory;
}
