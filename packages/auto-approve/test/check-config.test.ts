...
async function expectToThrow(fn: () => Promise<void>): Promise<void> {
  try {
    await fn();
  } catch (e) {
    return;
...
describe('check for config', () => {
  describe('whether config is a valid YAML object', () => {
    ...
    it('should not throw if YAML is valid', async () => {
      validateSchema(
        fs.readFileSync(
